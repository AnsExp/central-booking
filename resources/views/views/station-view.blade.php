...
@section('content')
    @php
        ...
        $nextLocationDirection = $sort === 'location' && $direction === 'asc' ? 'desc' : 'asc';
    @endphp
        ...
        <div
            ...
            <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6">
                <div>
                    ...
                    <p class="mt-4 text-slate-600 max-w-3xl text-base sm:text-lg">
                        Este es el módulo de estaciones. Desde aquí puedes ir directamente al módulo de edición.
                    </p>
                </div>
                @if ($stations->total() > 0)
                    <a href="{{ route('stations.form') }}"
            ...
            @if ($stations->total() > 0)
                <div class="mt-8 rounded-2xl border border-slate-200 bg-white overflow-hidden">
                            ...
                            <thead class="bg-slate-50 text-slate-600">
                                <tr>
                                    <th class="text-left px-4 py-3 font-semibold">
                                        <a href="{{ route('stations.view', ['sort' => 'name', 'direction' => $nextNameDirection]) }}"
                                    ...
                                    <th class="text-left px-4 py-3 font-semibold">
                                        <a href="{{ route('stations.view', ['sort' => 'location', 'direction' => $nextLocationDirection]) }}"
                                            class="inline-flex items-center gap-1 hover:text-slate-900 transition-colors">
                                            Ubicación
                                            @if ($sort === 'location')
                                                <span class="text-xs">
                                    ...
                                    <tr>
                                        ...
                                        <td class="px-4 py-3">
                                            <div class="flex items-center justify-end gap-2">
                                                <a href="{{ route('stations.form', ['station' => $station->id]) }}"
                                                ...
                                                <form method="POST" action="{{ route('stations.destroy', $station) }}"
                                                    onsubmit="return confirm('¿Deseas eliminar esta estación?');">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="submit"
                    ...
                    <x-pagination-controls :paginator="$stations" />
                </div>
            @else
                <div class="mt-8">
                    <x-empty-data title="No hay estaciones registradas"
                        message="Parece que aún no has agregado ninguna estación. Comienza creando una nueva para gestionar tu red ferroviaria."
                        action-label="Crear estación" action-href="{{ route('stations.form') }}" />
                </div>
            @endif
        </div>
    </div>
