@extends('layouts.app')

@section('content')
    @php
        use App\Models\TrainStatus;
        use App\Models\TrainType;

        $nextNameDirection = $sort === 'name' && $direction === 'asc' ? 'desc' : 'asc';
        $nextCodeDirection = $sort === 'code' && $direction === 'asc' ? 'desc' : 'asc';
        $nextTypeDirection = $sort === 'type' && $direction === 'asc' ? 'desc' : 'asc';
        $nextStatusDirection = $sort === 'status' && $direction === 'asc' ? 'desc' : 'asc';
    @endphp
    <div class="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-10">
        <div
            class="rounded-3xl border border-slate-200 bg-gradient-to-r from-cyan-50 via-white to-indigo-50 p-6 sm:p-8 lg:p-10 text-slate-900 shadow-sm">
            <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6">
                <div>
                    <h1 class="mt-4 text-3xl sm:text-5xl font-extrabold tracking-tight leading-tight">
                        Trenes
                    </h1>
                    <p class="mt-4 text-slate-600 max-w-3xl text-base sm:text-lg">
                        Este es el módulo de trenes. Desde aquí puedes ir directamente al módulo de edición.
                    </p>
                </div>
                @if ($trains->total() > 0)
                    <a href="{{ route('trains.form') }}"
                        class="inline-flex items-center justify-center rounded-full bg-slate-900 text-white px-6 py-3 text-sm font-semibold hover:bg-slate-800 transition-all">
                        <x-heroicon-o-plus class="w-5 h-5 mr-2" />
                        Nuevo
                    </a>
                @endif
            </div>
            @if ($trains->total() > 0)
                <div class="mt-8 rounded-2xl border border-slate-200 bg-white overflow-hidden">
                    <div class="overflow-x-auto">
                        <table class="table-fixed min-w-full text-sm">
                            <thead class="bg-slate-50 text-slate-600">
                                <tr>
                                    <th class="text-left px-4 py-3 font-semibold">
                                        <a href="{{ route('trains.view', ['sort' => 'name', 'direction' => $nextNameDirection]) }}"
                                            class="inline-flex items-center gap-1 hover:text-slate-900 transition-colors">
                                            Nombre
                                            @if ($sort === 'name')
                                                <span class="text-xs">
                                                    @if ($direction === 'asc')
                                                        <x-heroicon-o-arrow-small-down class="w-4 h-4" />
                                                    @else
                                                        <x-heroicon-o-arrow-small-up class="w-4 h-4" />
                                                    @endif
                                                </span>
                                            @endif
                                        </a>
                                    </th>
                                    <th class="text-left px-4 py-3 font-semibold">
                                        <a href="{{ route('trains.view', ['sort' => 'code', 'direction' => $nextCodeDirection]) }}"
                                            class="inline-flex items-center gap-1 hover:text-slate-900 transition-colors">
                                            Código
                                            @if ($sort === 'code')
                                                <span class="text-xs">
                                                    @if ($direction === 'asc')
                                                        <x-heroicon-o-arrow-small-down class="w-4 h-4" />
                                                    @else
                                                        <x-heroicon-o-arrow-small-up class="w-4 h-4" />
                                                    @endif
                                                </span>
                                            @endif
                                        </a>
                                    </th>
                                    <th class="text-left px-4 py-3 font-semibold">
                                        <a href="{{ route('trains.view', ['sort' => 'type', 'direction' => $nextTypeDirection]) }}"
                                            class="inline-flex items-center gap-1 hover:text-slate-900 transition-colors">
                                            Tipo
                                            @if ($sort === 'type')
                                                <span class="text-xs">
                                                    @if ($direction === 'asc')
                                                        <x-heroicon-o-arrow-small-down class="w-4 h-4" />
                                                    @else
                                                        <x-heroicon-o-arrow-small-up class="w-4 h-4" />
                                                    @endif
                                                </span>
                                            @endif
                                        </a>
                                    </th>
                                    <th class="text-left px-4 py-3 font-semibold">
                                        <a href="{{ route('trains.view', ['sort' => 'status', 'direction' => $nextStatusDirection]) }}"
                                            class="inline-flex items-center gap-1 hover:text-slate-900 transition-colors">
                                            Estado
                                            @if ($sort === 'status')
                                                <span class="text-xs">
                                                    @if ($direction === 'asc')
                                                        <x-heroicon-o-arrow-small-down class="w-4 h-4" />
                                                    @else
                                                        <x-heroicon-o-arrow-small-up class="w-4 h-4" />
                                                    @endif
                                                </span>
                                            @endif
                                        </a>
                                    </th>
                                    <th class="text-right px-4 py-3 font-semibold">Acciones</th>
                                </tr>
                            </thead>
                            <tbody class="divide-y divide-slate-100 text-slate-700">
                                @foreach ($trains as $train)
                                    <tr>
                                        <td class="px-4 py-3">{{ $train->name }}</td>
                                        <td class="px-4 py-3">{{ $train->code }}</td>
                                        <td class="px-4 py-3">{{ TrainType::label($train->type) }}</td>
                                        <td class="px-4 py-3">{{ TrainStatus::label($train->status) }}</td>
                                        <td class="px-4 py-3">
                                            <div class="flex items-center justify-end gap-2">
                                                <a href="{{ route('trains.form', ['train' => $train->id]) }}"
                                                    class="inline-flex items-center rounded-full bg-slate-100 text-slate-700 px-3 py-1.5 text-xs font-semibold hover:bg-slate-200 transition-all"
                                                    title="Editar tren">
                                                    <x-heroicon-o-pencil class="w-4 h-4 text-slate-600" />
                                                </a>

                                                <form method="POST" action="{{ route('trains.destroy', $train) }}"
                                                    onsubmit="return confirm('¿Deseas eliminar este tren?');">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="submit"
                                                        class="inline-flex items-center rounded-full bg-red-50 text-red-700 px-3 py-1.5 text-xs font-semibold hover:bg-red-100 transition-all"
                                                        title="Eliminar tren">
                                                        <x-heroicon-o-trash class="w-4 h-4" />
                                                    </button>
                                                </form>
                                            </div>
                                        </td>
                                    </tr>
                                @endforeach
                            </tbody>
                        </table>
                    </div>
                    <x-pagination-controls :paginator="$trains" />
                </div>
            @else
                <div class="mt-8">
                    <x-empty-data title="No hay trenes registrados"
                        message="Parece que aún no has agregado ningún tren. Comienza creando uno nuevo para gestionar tu red ferroviaria."
                        action-label="Crear tren" action-href="{{ route('trains.form') }}" />
                </div>
            @endif
        </div>
    </div>
@endsection