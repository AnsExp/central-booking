...
class CodeEditorComponent extends TextareaComponent
{
    ...
    public function compact(): string
    {
        ...
        ob_start();
        ?>
        <textarea id="<?= esc_attr($this->id) ?>" name="<?= esc_attr($this->get_attribute('name')) ?>" <?php foreach ($this->get_attributes() as $key => $value): ?>             <?php if ($key !== 'name'): ?>                 <?= esc_attr($key) ?>="<?= esc_attr($value) ?>" <?php endif; ?>         <?php endforeach; ?>
