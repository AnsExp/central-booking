...
class CounterComponent extends BaseComponent
{
    ...
    public function __construct(?int $min = null, ?int $max = null)
    {
        ...
        if ($min !== null) {
            $this->set_attribute('data-min-value', $min);
        }
        if ($max !== null) {
            $this->set_attribute('data-max-value', $max);
        }
        wp_enqueue_style(
            'central-tickets-counter-component',
            CENTRAL_BOOKING_URL . '/assets/css/components/counter-component.css',
            [],
            '1.0.0'
        );
        wp_enqueue_script(
            'central-tickets-counter-component',
            CENTRAL_BOOKING_URL . '/assets/js/components/counter-component.js',
            ['jquery'],
            '1.0.0',
            true
        );
