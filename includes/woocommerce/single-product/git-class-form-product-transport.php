...
class FormProductTransport implements Component
{
     ...
     */
    public static function query_transports(array $args)
    {
        $routes = FormProductRoute::query_routes($args);
        $transports = [];
        foreach ($routes as $route) {
            foreach ($route->get_transports() as $transport) {
                $transport->set_routes([$route]);
                $transports[$transport->id] = $transport;
            }
        }
        $transports = array_values($transports);
        if (isset($args['splitByAlias']) && !$args['splitByAlias']) {
            return self::sort_transports($transports);
        }
        foreach ($transports as $transport) {
            foreach ($transport->get_meta('alias') as $alias) {
     ...
     * @return array
     */
    private static function sort_transports(array $transports)
    {
        usort($transports, function (Transport $a, Transport $b) {
            return $a->nicename <=> $b->nicename;
        });
        foreach ($transports as $transport) {
            $services = $transport->get_services();
    ...
    private function get_zone(int $zone_id)
    {
        $repository = new ZoneRepository();
        return $repository->find($zone_id);
    }
    ...
    public function compact()
    {
        ...
        $prices = $calculator->get_prices($this->product);
        ...
        <div id="git-form-product-transport" style="display: none;">
            ...
            <?php
            $this->extra_component(
                'Flexible — $' . $prices[PriceExtraConstants::FLEXIBLE],
